...
class PMatches extends StatefulWidget {
  final bool hslashp;

  PMatches({this.hslashp = true});

  @override
  _PMatchesState createState() => _PMatchesState();
}

class _PMatchesState extends State<PMatches> {
    ...
    if (userSnapshot.exists) {
      setState(() {
        _conductedTournaments =
            List<String>.from(userSnapshot.get('participatedMatches') ?? []);
      });
    }
  }
  ...
  Widget _buildTournamentsGrid() {
    ...
    if (_conductedTournaments.isEmpty) {
      ...
      return Scaffold(
        appBar: AppBar(
          title: Text('Participated Matches'),
          leading: IconButton(
            onPressed: () => ZoomDrawer.of(context)!.toggle(),
            icon: Icon(Icons.menu),
          ),
